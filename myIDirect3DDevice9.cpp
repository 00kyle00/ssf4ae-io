...
IDirect3DDevice9* olddev = 0;

typedef void __stdcall D3DXCreateTextureFromFile_t(IDirect3DDevice9*, const char*, IDirect3DTexture9**);
...
void DrawIcons(IDirect3DDevice9* dev, bool leftSide, int ypos, unsigned state) {
  ...
  if(arrows != 0) {
    if(((arrows & 0x1) && (arrows & 0x2)) ||
      ((arrows & 0x4) && (arrows & 0x8)))
    {
      exit(0);
      DrawIcon(dev,  1,  1, 0);
    }
...
{
  if(m_pIDirect3DDevice9 != olddev) {
    m_pIDirect3DDevice9->CreateVertexBuffer(
      sizeof(vertices_org), D3DUSAGE_WRITEONLY, D3DFVF_CUSTOMVERTEX, D3DPOOL_DEFAULT, 
      &vertexBuffer, NULL );

    HMODULE d3dx9 = LoadLibrary("d3dx9_24.dll");
    D3DXCreateTextureFromFile_t* ctff = (D3DXCreateTextureFromFile_t*)
      GetProcAddress(d3dx9, "D3DXCreateTextureFromFileA");

    ctff(m_pIDirect3DDevice9, "icons.dds", &texture);
    m_pIDirect3DDevice9->GetViewport(&viewport);
    aspect_ratio = viewport.Width;
    aspect_ratio /= viewport.Height;
    olddev = m_pIDirect3DDevice9;
  }

  SetupRenderstates(m_pIDirect3DDevice9);
  static std::deque<unsigned> history(15);

  if(GetAsyncKeyState(VK_F12))
    history = std::deque<unsigned>(history.size());

  static unsigned last_input = 0;
  ...
  if((buttons | stick) != 0) {
    history.push_front(computed_input);
    history.pop_back();
  }

  for(int i=0; i<history.size(); ++i)
    DrawIcons(m_pIDirect3DDevice9, true, i, history[i]);
